...
const WORKOUT_CONFIGS = {
  push:     { label: 'Push Day',      color: '#ff375f', muscles: ['chest','shoulders','triceps'] },
  pull:     { label: 'Pull Day',      color: '#0a84ff', muscles: ['back','biceps','forearms'] },
  legs:     { label: 'Leg Day',       color: '#ffd60a', muscles: ['quads','hamstrings','glutes','calves'] },
  upper:    { label: 'Upper Body',    color: '#bf5af2', muscles: ['chest','back','shoulders','biceps','triceps'] },
  lower:    { label: 'Lower Body',    color: '#ff9f0a', muscles: ['quads','hamstrings','glutes','calves','core'] },
  fullbody: { label: 'Full Body',     color: '#30d158', muscles: ['chest','back','quads','hamstrings','shoulders','core'] },
  rest:     { label: 'Rest Day',      color: '#3a3a4a', muscles: [] },
}
  ...
  return (
    <div className="px-4 pb-6 space-y-5 animate-fade-in">
      ...
      <div className="bg-gradient-to-br from-card to-surface border border-border rounded-3xl p-4 relative overflow-hidden">
        ...
        <div className="flex gap-3 relative">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-[10px] font-mono text-dim tracking-wider uppercase mb-1">AI Coach</p>
            {greetLoad ? (
              ...
              <p className="text-subtle text-sm">Tap Coach tab for personalized insights.</p>
            )}
          </div>
        </div>
      </div>
        ...
        <div className="relative flex items-center gap-3">
          ...
          <div className="flex-1">
            <p className="text-[10px] font-mono tracking-wider uppercase mb-0.5"
              style={{ color: todayConfig.color }}>TODAY</p>
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          { label: 'This Week',   value: weeklyWorkouts.length,         unit: 'sessions', icon: <TrendingUp className="w-4 h-4" />, color: '#0a84ff' },
          { label: 'Volume',      value: Math.round(weeklyVolume / 1000 * 10) / 10, unit: 'ton', icon: <Zap className="w-4 h-4" />, color: '#ff9f0a' },
          { label: 'Readiness',   value: todayReadiness,                unit: '%',   icon: <Clock className="w-4 h-4" />,       color: todayReadiness > 70 ? '#30d158' : '#ff375f' },
        ].map(s => (
      ...
      <div className="bg-card border border-border rounded-3xl p-4">
        <div className="flex items-center justify-between mb-3">
          <p className="text-white font-bold text-sm">Muscle Status</p>
          <button onClick={() => { haptic([25]); setTab('library') }}
            className="text-blue text-xs font-mono flex items-center gap-1">
            View exercises <ChevronRight className="w-3 h-3" />
          </button>
        </div>
      ...
      {rankedMuscles.length > 0 && (
        <div className="bg-card border border-border rounded-3xl p-4">
          <p className="text-white font-bold text-sm mb-3">Recovery Status</p>
      ...
      {/* ── Week overview strip ──────────────────────────────────────────────── */}
      <div>
        <p className="text-white font-bold text-sm mb-3">This Week</p>
        <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-none">
          {DAYS.map((day, idx) => {
            ...
            return (
              <div key={day} className={`flex-shrink-0 flex flex-col items-center gap-1.5 rounded-2xl p-2.5 transition-all ${
                isToday ? 'border-2' : 'border border-border bg-card'
              }`} style={isToday ? {
